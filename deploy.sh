...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=maxpicky/multi-server-k8s:$GIT_SHA
kubectl set image deployments/client-deployment client=maxpicky/multi-client-k8s:$GIT_SHA
kubectl set image deployments/worker-deployment worker=maxpicky/multi-worker-k8s:$GIT_SHA